...
class LinkedList
  ...
  def delete(value)
    ...
    #     # need to know the next node
    #     # set the previous node to this next node --> knowing index/position would be nice
  ...
  def find_nth_from_end(n)
    puts "Not implemented"
    # want to know length
    # approach one - find length - then length - n - 1 is index
    # iterate through the list again to the desired index/position

    # approach two - tranverse just once
    # current and trailing tracking - trailing starts once current reaches the value you want,
    # then when current reaches the end, the trailing count is the index from the end
...
puts "BUG: Middle value should be 3 and not #{middle}" if middle != 3
#
